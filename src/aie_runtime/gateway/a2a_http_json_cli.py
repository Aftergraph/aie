...
from .a2a_http_json import (
    A2AHTTPJSONForwarder,
    A2AHTTPJSONServer,
    create_a2a_http_json_server,
)
        ...
        if bool(workload.get("watch", False)):
            provider = RotatingTLSContextProvider(
                ...
                require_client_cert=bool(
                    (listen.get("tls") or {}).get("require_client_cert", True)
                ),
            )
        ...
        if listen_tls.get("source") == "workload_api":
            if provider is not None:
                ...
                inbound_context, _ = build_ssl_contexts_from_svid(
                    material,
                    require_client_cert=bool(
                        listen_tls.get("require_client_cert", True)
                    ),
                )
            else:
                raise ValueError(
                    "workload_api listen TLS requires workload_api configuration"
                )
        else:
    ...
    if upstream_tls:
        if upstream_tls.get("source") == "workload_api":
            if provider is not None:
                ...
                _, outbound_context = build_ssl_contexts_from_svid(
                    material,
                    require_client_cert=True,
                )
            else:
                raise ValueError(
                    "workload_api upstream TLS requires workload_api configuration"
                )
        else:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="AIE A2A 1.0 HTTP+JSON transport gateway"
    )
