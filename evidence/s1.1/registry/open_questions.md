...
6. **What are the 61 checks in `checks_total=195` that aren't in `check_ids`?** The promotion report shows `check_ids: 134` and `checks_total: 195`. The 61 extra checks are likely parameterized variants or implicit checks. This is a reporting gap, not a correctness issue. The S2 comparator (`interop/s2/collect_report.py::validate_s1_attestation`) requires `checks_total == len(check_ids)`, so the S1 report would fail S2 validation until this is reconciled.
7. **Does the S2 comparator need to accept `leg.status == "PASS_UPSTREAM_GAP"`?** The S1.1 promotion has legs with `status: "PASS_UPSTREAM_GAP"` (correct demotion for upstream MCP issues). The S2 comparator requires `leg.status == "PASS"`. This needs to be reconciled before S2 can run.
