...
def test_workload_api_watch_drives_inbound_and_outbound_tls(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "aie_runtime.gateway.a2a_http_json_cli.WorkloadAPIClient",
        FakeClient,
    )
    config = _base_config(tmp_path)
    config["workload_api"] = {
    ...
    try:
        ...
        assert (
            built.server.forwarder.ssl_context_provider()
            is built.server.tls_context_provider.client_context()
        )
        assert (
            built.server.forwarder.expected_peer_spiffe_id
            == "spiffe://example.org/a2a/server"
        )
