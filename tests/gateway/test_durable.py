...
def _expected_revocation_digest(rows):
    ...
    Per SDD spec section 6.2, canonical rows include lease_id, revoked_at,
    and source_gateway.
    """
    canonical = [{"lease_id": r["lease_id"], "revoked_at": r["revoked_at"], "source_gateway": r["source_gateway"]} for r in rows]
