...
run_as aie-s1-server env AIE_BRIDGE_DEBUG=1 "$AIE_S1_PYTHON" -m aie_runtime.gateway.bridge_cli --config "$SERVER_BRIDGE_CONFIG" >"$AIE_S1_STATE/logs/server-bridge.log" 2>&1 & echo $! >"$AIE_S1_STATE/server-bridge.pid"
AIE_GATEWAY_ADMIN_TOKEN=s1-admin run_as aie-s1-gateway env AIE_GATEWAY_ADMIN_TOKEN=s1-admin AIE_GATEWAY_DEBUG=1 "$AIE_S1_PYTHON" -m aie_runtime.gateway.cli --config "$HERE/config/gateway.json" --host 127.0.0.1 --port 18444 >"$AIE_S1_STATE/logs/gateway.log" 2>&1 & echo $! >"$AIE_S1_STATE/gateway.pid"
...
run_as aie-s1-client env AIE_BRIDGE_DEBUG=1 "$AIE_S1_PYTHON" -m aie_runtime.gateway.bridge_cli --config "$CLIENT_BRIDGE_AIE_CONFIG" >"$AIE_S1_STATE/logs/client-bridge-aie.log" 2>&1 & echo $! >"$AIE_S1_STATE/client-bridge-aie.pid"
